...
/**
 * Servlet implementation class FacultyLoginCtl
 */
@WebServlet(name="FacultyLoginCtl",urlPatterns={"/home/login/faculty"})
public class FacultyLoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (id > 0) {
			...
			ServletUtility.forward(ELearnView.FACULTY_LOGIN_VIEW, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(ELearnView.Faculty_Dashboard_CTL, request, response);
			return;
		}
...
HttpSession session = request.getSession(true);
	...
	protected String getView() {
		...
		return ELearnView.FACULTY_LOGIN_VIEW;
	}
